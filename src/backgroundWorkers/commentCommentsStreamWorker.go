		...
		for {
			...
			for parentCommentId, cc := range parentCommentComments {
				...
				sqls = append(
					sqls,
					/* sql */ `
					UPDATE public.comments SET comments_count = comments_count + $2 WHERE comment_id = $1
					RETURNING comment_id, comments_count
					`,
				)
			...
			type res struct {
				ParentCommentId string `db:"comment_id"`
				CommentsCount   int    `db:"comments_count"`
			}

			parentCommentIdComments, err := pgDB.BatchQueryTypeTx[res](ctx, pgTx, sqls, params)
			if err != nil {
				helpers.LogError(err)
				return
			}
