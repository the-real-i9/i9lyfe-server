...
func (b changeProfilePictureBody) Validate(ctx context.Context) error {
	...
	go func(ppicCn string) {
		...
		if mInfo := mediaStorageService.GetMediaInfo(ctx, smallPPicCn); mInfo != nil {
			if mInfo.Size < 1*1024 || mInfo.Size > 500*1024 {
				mediaStorageService.DeleteCloudMedia(ctx, smallPPicCn)
			}
		}

		if mInfo := mediaStorageService.GetMediaInfo(ctx, mediumPPicCn); mInfo != nil {
			if mInfo.Size < 1*1024 || mInfo.Size > 1*1024*1024 {
				mediaStorageService.DeleteCloudMedia(ctx, mediumPPicCn)
			}
		}

		if mInfo := mediaStorageService.GetMediaInfo(ctx, largePPicCn); mInfo != nil {
			if mInfo.Size < 1*1024 || mInfo.Size > 2*1024*1024 {
				mediaStorageService.DeleteCloudMedia(ctx, largePPicCn)
			}
		}
