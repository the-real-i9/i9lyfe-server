	...
	if mediaCloudName := m.MediaCloudName; mediaCloudName != nil {
		go func(msgType, mediaCloudName string) {
			...
			case "photo", "video":
				...
				if mInfo := mediaStorageService.GetMediaInfo(ctx, mcnBlur); mInfo != nil {
					if mInfo.Size < 1*1024 || mInfo.Size > 100*1024 {
						mediaStorageService.DeleteCloudMedia(ctx, mcnBlur)
					}
				}

				if mInfo := mediaStorageService.GetMediaInfo(ctx, mcnActual); mInfo != nil {
					if msgType == "photo" && mInfo.Size < 1*1024 || mInfo.Size > 10*1024*1024 {
						mediaStorageService.DeleteCloudMedia(ctx, mcnActual)
					} else if mInfo.Size < 1*1024 || mInfo.Size > 40*1024*1024 {
						mediaStorageService.DeleteCloudMedia(ctx, mcnActual)
					}
				}
			case "voice":
				if mInfo := mediaStorageService.GetMediaInfo(ctx, mediaCloudName); mInfo != nil {
					if mInfo.Size < 500 || mInfo.Size > 10*1024*1024 {
						mediaStorageService.DeleteCloudMedia(ctx, mediaCloudName)
					}
				}
			case "audio":
				if mInfo := mediaStorageService.GetMediaInfo(ctx, mediaCloudName); mInfo != nil {
