...
func BuildPostUIFromCache(ctx context.Context, postId, clientUsername string) (postUI UITypes.Post, err error) {
	...
	postUI.MediaUrls = mediaStorageService.PostMediaCloudNamesToUrl(postUI.MediaUrls)
	...
	pud.OwnerUser.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(pud.OwnerUser.ProfilePicUrl)

	postUI.OwnerUser = pud.OwnerUser

	if postUI.ReposterUser != nil {
		pud.ReposterUser.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(pud.ReposterUser.ProfilePicUrl)
...
func BuildCommentUIFromCache(ctx context.Context, commentId, clientUsername string) (commentUI UITypes.Comment, err error) {
	...
	commentUI.AttachmentUrl = mediaStorageService.CommentAttachCloudNameToUrl(commentUI.AttachmentUrl)
	...
	cud.OwnerUser.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(cud.OwnerUser.ProfilePicUrl)
...
func buildUserSnippetUIFromCache(ctx context.Context, username, clientUsername string) (userSnippetUI UITypes.UserSnippet, err error) {
	...
	usud.User.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(usud.User.ProfilePicUrl)
...
func buildReactorSnippetUIFromCache(ctx context.Context, username, postOrComment, entityId string) (reactorSnippetUI UITypes.ReactorSnippet, err error) {
	...
	rsud.User.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(rsud.User.ProfilePicUrl)
...
func BuildUserProfileUIFromCache(ctx context.Context, username, clientUsername string) (userProfileUI UITypes.UserProfile, err error) {
	...
	upud.User.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(upud.User.ProfilePicUrl)
...
func BuildNotifSnippetUIFromCache(ctx context.Context, notifId string) (notifSnippetUI UITypes.NotifSnippet, err error) {
	...
	setNotifUserDetail := func(userKey string) error {
		...
		user.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(user.ProfilePicUrl)
...
func buildChatSnippetUIFromCache(ctx context.Context, clientUsername, partnerUser string) (chatSnippetUI UITypes.ChatSnippet, err error) {
	...
	csuipu.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(csuipu.ProfilePicUrl)
...
func BuildCHEUIFromCache(ctx context.Context, CHEId string) (CHEUI UITypes.ChatHistoryEntry, err error) {
	...
	if CHEUI.CHEType == "message" {
		...
		cuis.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(cuis.ProfilePicUrl)

		CHEUI.Sender = cuis

		mediaStorageService.MessageMediaCloudNameToUrl(CHEUI.Content)
		...
		for user, emoji := range userEmojiMap {
			...
			uimsgr.ProfilePicUrl = mediaStorageService.ProfilePicCloudNameToUrl(uimsgr.ProfilePicUrl)
